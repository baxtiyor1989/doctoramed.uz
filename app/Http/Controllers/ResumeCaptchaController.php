...
class ResumeCaptchaController extends Controller
{
    public function __invoke(Request $request): Response
    {
        $code = (string) random_int(10000, 99999);
        $codes = collect($request->session()->get('resume_captcha_codes', []))
            ->filter(fn (array $item) => ($item['expires_at'] ?? 0) >= now()->timestamp)
            ->push(['code' => $code, 'expires_at' => now()->addMinutes(5)->timestamp])
            ->take(-3)->values()->all();
        $request->session()->put('resume_captcha_codes', $codes);
        ...
        return response($content, 200, ['Content-Type' => 'image/png', 'Cache-Control' => 'no-store, no-cache, must-revalidate, max-age=0']);
    }
}
